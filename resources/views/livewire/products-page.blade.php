            ...
            <div class="flex flex-col lg:flex-row lg:space-x-6 rtl:space-x-reverse" x-data="{ open: false }">
                ...
                <div class="w-full lg:w-1/4 mb-6 lg:mb-0">
                    ...
                    <div class="flex justify-between items-center mb-4 lg:hidden">
                        <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-200">{{ __('product.filters') }}
                        </h2>
                    ...
                    <div class="hidden lg:block">
                        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow">
                            <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">{{
                                __('product.filters') }}</h2>

                            <!-- Category Filter -->
                            {{-- <!-- Category Filter -->
                            <div class="mb-6">
                                <h3 class="text-lg font-medium text-gray-700 dark:text-gray-200 mb-2">{{
                                    __('product.category') }}</h3>
                                <div class="space-y-2 max-h-60 overflow-y-auto">
                                    @foreach($categories as $category)
                                    <label class="block">
                                        <!-- This ensures each item is stacked vertically -->
                                        <input type="checkbox" wire:model.live="selectedCategories"
                                            value="{{ $category->id }}" class="form-checkbox h-4 w-4 text-blue-600">
                                        <span class="ml-2 text-gray-700 dark:text-gray-300">{{
                                            $category->translations->first()->name }}d</span>
                                    </label>
                                    @endforeach
                                </div>
                            </div> --}}
                            ...
                            <div class="mb-6">
                                ...
                                <div class="space-y-2 max-h-60 overflow-y-auto">
                                    @foreach($categories as $category)
                                    @include('livewire.partials.category-checkbox', ['category' => $category, 'locale'
                                    => app()->getLocale()])
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="space-y-2 max-h-60 overflow-y-auto">
                                    ...
                                    <label class="block">
                                        ...
                                        <span class="ml-2 text-gray-700 dark:text-gray-300">{{
                                            $brand->translations->first()->name }}</span>
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                <h3 class="text-lg font-medium text-gray-700 dark:text-gray-200 mb-2">{{
                                    __('product.price_range') }}</h3>
                                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                    <input type="number" wire:model.debounce.500ms="priceRange.0"
                                        class="w-20 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        placeholder="{{ __('product.min') }}" min="0">
                                    <span class="text-gray-700 dark:text-gray-300">-</span>
                                    <input type="number" wire:model.debounce.500ms="priceRange.1"
                                        class="w-20 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        placeholder="{{ __('product.max') }}" min="0">
                                </div>
                            </div>
                    ...
                    <div class="fixed inset-0 z-40 lg:hidden" x-show="open"
                        ...
                        <div
                            class="absolute top-0 left-0 w-3/4 max-w-xs bg-white dark:bg-gray-900 h-full shadow-lg p-6 overflow-y-auto">
                                ...
                                <div class="space-y-2 max-h-60 overflow-y-auto">
                                    ...
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" wire:model.live="selectedCategories"
                                            value="{{ $category->id }}" class="form-checkbox h-4 w-4 text-blue-600">
                                        <span class="ml-2 text-gray-700 dark:text-gray-300">{{
                                            $category->translations->first()->name }}</span>
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                <h3 class="text-lg font-medium text-gray-700 dark:text-gray-200 mb-2">{{
                                    __('product.price_range') }}</h3>
                                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                    <input type="number" wire:model.live="priceRange.0"
                                        class="w-20 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        placeholder="{{ __('product.min') }}" min="0">
                                    <span class="text-gray-700 dark:text-gray-300">-</span>
                                    <input type="number" wire:model.debounce.500ms="priceRange.1"
                ...
                <div class="w-full lg:w-3/4">
                    ...
                    <div class="flex justify-between items-center mb-4 rtl:flex-row-reverse rtl:space-x-reverse">
                        <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-200">{{ __('product.products') }}
                        </h2>
                        <!-- Sort By Dropdown for Large Screens -->
                        <div class="hidden lg:block">
                            <label class="block text-lg font-medium text-gray-700 dark:text-gray-200">{{
                                __('product.sort_by') }}</label>
                            <select wire:model.live="sortBy"
                        ...
                        <div
                            ...
                            <a class="relative mx-2 mt-2 flex h-48 overflow-hidden rounded-xl"
                                ...
                                <img class="absolute inset-0 h-full w-full object-cover transition-opacity duration-500 peer-hover:opacity-0"
                                    ...
                                    alt="{{ $product->translation->name ?? 'Product Image' }}">
                                @else
                                <img class="absolute inset-0 h-full w-full object-cover"
                                    src="https://via.placeholder.com/300" alt="Placeholder">
                                @endif
                                @if ($secondImage)
                                ...
                                @php
                                $discountPercentage = round((($product->price - $product->sale_price) / $product->price)
                                * 100);
                                @endphp
                                <span
                            ...
                            <div class="mt-0 px-2 pb-5 text-center">
                                <a href="/products/{{$product->slug}}">
                                    <h5 class="text-xs text-gray-700 dark:text-gray-200">{{ $product->translation->name
                                        ?? 'No name available' }}</h5>
                                </a>
                                <div class="mt-2">
                                    @if ($product->sale_price)
                                    <span class="text-sm font-bold text-red-500">{{ number_format($product->sale_price)
                                        }} {{ __('general.currency_symbol') }}</span>
                                    <span class="text-sm font-semibold text-gray-400 line-through ml-2">{{
                                        number_format($product->price) }}</span>
                                    @else
                                    <span class="text-sm font-bold text-blue-500">{{ number_format($product->price) }}
                                        {{ __('general.currency_symbol') }}</span>
                                    @endif
                                </div>
                                ...
                                <div class="rating-stars flex justify-center my-2">
                                    ...
                                    @for ($i = 0; $i < $fullStars; $i++) <i class="fa fa-star text-yellow-400"></i>
                                        @endfor
                                        ...
                                        @for ($i = 0; $i < $emptyStars; $i++) <i class="fa fa-star text-gray-300"></i>
                                            @endfor
                                </div>
                                ...
                                <a href="#" wire:click.prevent="addToCart({{ $product->id }})"
                                    class="hover:bg-blue-700 hover:ring-2 hover:ring-blue-400 transition duration-300 flex items-center justify-center rounded-md bg-blue-600 px-3 py-2 text-xs font-medium text-white focus:outline-none focus:ring-4 focus:ring-blue-300 font-semibold">
                                    <span wire:loading.remove wire:target="addToCart({{ $product->id }})">
                                        {{ __('product.add_to_cart') }}
                                    </span>
                                    <span wire:loading wire:target="addToCart({{ $product->id }})"
                                        class="flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="animate-spin mr-2 h-4 w-4"
