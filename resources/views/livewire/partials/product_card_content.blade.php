<div class="">
    <div class="group relative border-gray-200 dark:border-gray-700 flex flex-col w-full max-w-xs overflow-hidden rounded-lg border bg-white dark:bg-gray-900 shadow-md transition-transform transform hover:scale-105">
        <a class="relative mx-2 mt-2 flex h-48 overflow-hidden rounded-xl" href="{{ locale_route('products.show', ['slug' => $product->slug]) }}"">
            ...
            <span class="absolute top-2 left-2 bg-red-600 text-white text-xs font-semibold px-2 py-1 rounded">{{ __('product.sale') }}</span>
        @endif
            @if (!$product->isInStock())
                <span class="absolute top-2 right-2 bg-red-600 text-white text-xs font-semibold px-2 py-1 rounded">{{ __('product.out_of_stock') }}</span>
            @endif
        ...
        <div class="mt-0 px-2 pb-5 text-center">
            ...
            <div class="mt-0 mb-2">
                @if ($product->sale_price)
                    <span class="text-lg font-bold text-red-500">{{ number_format($product->sale_price) }} {{ __('general.currency_symbol') }}</span>
                    <span class="text-sm text-gray-400 line-through ml-2">{{ number_format($product->price) }}</span>
                @else
                    <span class="text-lg font-bold text-blue-500">{{ number_format($product->price) }} {{ __('general.currency_symbol') }}</span>
                @endif
            </div>
