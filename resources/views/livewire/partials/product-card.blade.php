<div class="group relative border-gray-200 dark:border-gray-700 flex flex-col w-full max-w-xs overflow-hidden rounded-lg border bg-white dark:bg-gray-900 shadow-md transition-transform transform hover:scale-105">
    <a class="relative mx-2 mt-2 flex h-48 overflow-hidden rounded-xl" href="{{ route('products.show', ['locale' => app()->getLocale(), 'slug' => $product->slug]) }}">
        @php
            ...
            // $secondImage = $images[1] ?? null;
        @endphp
        <img class="absolute inset-0 h-full w-full object-cover transition-opacity duration-500 peer-hover:opacity-0" src="{{ asset('storage/' . $firstImage) }}" alt="{{ $product->translation->name ?? __('product.no_name_available') }}">
        @if ($secondImage)
            <img class="absolute inset-0 h-full w-full object-cover transition-opacity duration-500 opacity-0 peer-hover:opacity-100" src="{{ asset('storage/' . $secondImage) }}" alt="{{ $product->translation->name ?? __('product.no_name_available') }}">
        @endif
    ...
    <div class="mt-0 px-2 pb-5 text-center">
        ...
        <div class="mt-2">
            @if ($product->sale_price)
                ...
                <span class="text-sm font-bold text-gray-400 line-through ml-2">{{ number_format($product->price) }} </span>
            @else
        ...
        @if ($product->isInStock())
            <a href="#" wire:click.prevent="addToCart({{ $product->id }})" class="hover:bg-blue-700 hover:ring-2 hover:ring-blue-400 transition duration-300 flex items-center justify-center rounded-md bg-blue-600 px-3 py-2 text-xs font-medium text-white">
                <span wire:loading.remove wire:target="addToCart({{ $product->id }})">{{ __('product.add_to_cart') }}</span>
                <span wire:loading wire:target="addToCart({{ $product->id }})" class="flex items-center">
                    ...
                    </svg> {{ __('product.adding') }}
                </span>
            </a>
        @else
            <button class="rounded-md bg-gray-400 px-3 py-2 text-xs font-medium text-white cursor-not-allowed" disabled>{{ __('product.out_of_stock') }}</button>
        @endif
    </div>
</div>
