@if ($category->id != $categoryId) <!-- Ensure current category is not included -->
    <option value="{{ $category->id }}" {{ $parentCategoryId == $category->id ? 'selected' : '' }}>
        ...
        @foreach ($availableLocales as $locale)
            {{ strtoupper($locale) }}: {{ $category->translations->where('locale', $locale)->first()->name ?? 'Unnamed Category' }}
        @endforeach
    </option>

    @if ($category->children->isNotEmpty()) <!-- Check if there are child categories -->
